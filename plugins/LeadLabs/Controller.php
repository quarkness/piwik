 ...
 * @package Piwik_LeadLabs
 */
class Piwik_LeadLabs_Controller extends Piwik_Controller
{
	...
	public function getVisitorLog($fetch = false)
	{
		...
		$view->init( $this->pluginName,
							__FUNCTION__,
						'LeadLabs.getLastVisitsDetails'
						);
		$view->disableGenericFilters();
		$view->disableSort();
		$view->setTemplate("LeadLabs/templates/visitorLog.tpl");
		...
		$view->setReportDocumentation(Piwik_Translate('LeadLabs_VisitorLogDocumentation', array('<br />', '<br />')));
	...
	public function getLastVisitsStart($fetch = false)
	{
		...
		$api = new Piwik_API_Request("method=LeadLabs.getLastVisitsDetails&idSite=$this->idSite&filter_limit=10&format=php&serialize=0&disable_generic_filters=1");
	...
	{
		$last30min = Piwik_LeadLabs_API::getInstance()->getCounters($this->idSite, $lastMinutes = 30);
		$last30min = $last30min[0];
		$today = Piwik_LeadLabs_API::getInstance()->getCounters($this->idSite, $lastMinutes = 24*60);
		$today = $today[0];
